# Example: one system spin + M bath spins.
# Algorithm: split process-tensor `evolve(pt, ρ₀)` with fused bath memory.
# Reference: exact joint `exp(t·L)` via `liouvillian_propagator_itensor(..., Exact())`, then partial trace.
#
# From repo root:
#   julia scripts/pt_tfim_multimode.jl
...
function main(; dt::Float64=0.1, nsteps::Int=24, nmodes::Int=4)
    println("=== Process tensor ($nmodes bath modes): split PT vs joint Liouville ED ===")
    ...
    T_max = dt * nsteps
    println("Parameters: Δt=$dt, nsteps=$nsteps, T=$T_max, nmodes=$nmodes, 1 system spin")

    println("Building process tensor (fused bath memory)...")
    pt = build_process_tensor(system, system.sites[1]; environment=bath, dt=dt, nsteps=nsteps)
    println("  PT built ($(nsteps) slabs, Δt=$dt, $nmodes bath modes)")
    ...
    H_full += 1.0, "Sx", 1
    for m in 1:nmodes
        H_full += mode_w[m], "Sx", m + 1
        H_full += mode_g[m], "Sz", m + 1, "Sz", 1
    end
    ...
    mkpath(outdir)
    path = joinpath(outdir, "pt_tfim_multimode.png")
    save(path, fig)
    println("Saved: ", path)
    println(
...
main(; dt=0.1, nsteps=24, nmodes=4)
