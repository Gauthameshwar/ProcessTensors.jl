...
# Dict to access the particle type of the physical indices with their corresponding Liouville indices id
# Example storage: _LIOUV_PHYS_MAP[0x2da] = (dim=5|id=725|"Boson,Site,n=1")
const _LIOUV_PHYS_MAP = Dict{UInt64, Index}()

# Strips the particle type from the tags for easy mapping of Liouville sites to physical sites
function _liouv_tagset(s::Index)
    tag_tokens = string.(collect(tags(s)))
    phys = nothing
    for token in tag_tokens
        if token == "Site" || token == "Liouv" || startswith(token, "n=") || startswith(token, "l=")
            continue
        end
        phys = token
        break
    ...
    rest = filter(t -> t != "Liouv" && t != phys, tag_tokens)
    return ITensors.TagSet(join(vcat(["Liouv"], rest), ","))
end

_is_liouv_site(s::Index) = haskey(_LIOUV_PHYS_MAP, UInt64(ITensors.id(s))) || occursin("Liouv", string(tags(s)))
...
function to_liouville(
    ...
    if sites === nothing
        ...
        liouv_site_inds = [uniqueind(liouv_tensors[i], ρ.core[i]) for i in eachindex(liouv_tensors)]
        # Register each newly created Liouville index to recover the original physical SiteType in op("...", SiteType"Liouv", ...).
        for i in eachindex(liouv_site_inds)
            _LIOUV_PHYS_MAP[UInt64(ITensors.id(liouv_site_inds[i]))] = phys_inds[i][1]
        end
    else
        ...
        # Register provided Liouville indices so later Liouville-operator calls can map back to physical SiteTypes.
        for i in eachindex(sites)
            _LIOUV_PHYS_MAP[UInt64(ITensors.id(sites[i]))] = phys_inds[i][1]
        end
    end
...
**Returns:** Vector of Liouville indices (one per site), tagged with \"Liouv\"
"""
function liouv_sites(physical_sites::AbstractVector{<:Index})
    liouv = [Index(dim(s)^2; tags=_liouv_tagset(s)) for s in physical_sites]
    # Register Liouville<->physical index mapping at creation time (single source of truth).
    for i in eachindex(liouv)
        _LIOUV_PHYS_MAP[UInt64(ITensors.id(liouv[i]))] = physical_sites[i]
    end
    return liouv
end

# Resolve the original physical index from a Liouville index through the internal id->Index map.
function _phys_site_from_liouv(s::Index)
    s_phys = get(_LIOUV_PHYS_MAP, UInt64(ITensors.id(s)), nothing)
    s_phys === nothing && throw(
        ArgumentError(
            "No physical-site mapping found for Liouville index $(s). "
            * "Create Liouville sites with liouv_sites(physical_sites), or pass sites=... to to_liouville "
            * "using those same Liouville indices.",
        ),
    )
    return s_phys
end
...
# Helper to determine if sites are physical or Liouville and extract the correct set of Liouville sites for MPO construction.
...
# Tuple format support for jump operators (normalized to a vector of tuples for internal processing)
...
# OpSum format support for MPO_Liouville (normalized to a vector of OpSums for internal processing)
